...
import { CircleDollarSign, HelpCircle, Info, Loader2, RotateCw } from "lucide-react";
    ...
    <Layout>
      <div className="container py-10 max-w-4xl">
        ...
        <p className="text-lg text-muted-foreground mb-8">
          Add a new token with oracle integration to establish a virtual liquidity pool
        </p>
          ...
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>Configure the parameters for your pool</CardDescription>
              </div>
            </div>
          </CardHeader>
          ...
          <form onSubmit={handleSubmit}>
            <CardContent className="space-y-6">
              <div className="space-y-4">
                <div>
                  <Label htmlFor="tokenSymbol" className="mb-1 flex items-center gap-2">
                    Token Symbol
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p className="max-w-xs">Enter the symbol of the token you want to create a virtual pool for (e.g., ETH, LINK).</p>
                      </TooltipContent>
                    </Tooltip>
                  </Label>
                  <div className="flex gap-4 items-center">
                    <Input 
                      ...
                      required
                    />
                    {tokenSymbol && <TokenLogo symbol={tokenSymbol} size="md" />}
                  </div>
                </div>
                
                <div>
                  <Label htmlFor="initialLiquidity" className="mb-1 flex items-center gap-2">
                    Initial Liquidity
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <HelpCircle className="h-4 w-4 text-muted-foreground" />
                      ...
                      <TooltipContent>
                        <p className="max-w-xs">The amount of VLQT token to provide as initial liquidity for this virtual pool.</p>
                      </TooltipContent>
                    </Tooltip>
                  </Label>
                  <Input 
                ...
                <div className="space-y-2">
                  <Label htmlFor="oracleType" className="mb-1 flex items-center gap-2">
                    Oracle Type
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <HelpCircle className="h-4 w-4 text-muted-foreground" />
                      ...
                      <TooltipContent>
                        <p className="max-w-xs">Select the type of oracle to use for price feeds.</p>
                      </TooltipContent>
                    </Tooltip>
                  </Label>
                ...
                <div className="space-y-2">
                  <Label htmlFor="oracleAddress" className="mb-1 flex items-center gap-2">
                    Oracle Endpoint URL
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <HelpCircle className="h-4 w-4 text-muted-foreground" />
                      ...
                      <TooltipContent>
                        <p className="max-w-xs">Enter the oracle endpoint URL that will provide price data for this token.</p>
                      </TooltipContent>
                    </Tooltip>
                  </Label>
                  <div className="flex gap-2">
                    <Input 
                      ...
                      placeholder={oracleType === "api" ? "https://api.example.com/prices/{token}" : "..."}
                      required
                    />
                    <Button 
                      type="button" 
                      ...
                      disabled={isTestingOracle || !oracleAddress || !tokenSymbol}
                    >
                      {isTestingOracle ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <RotateCw className="h-4 w-4 mr-2" />}
                      Test
                    </Button>
                  </div>
                  
                  {oracleTestResult && (
                    <div className={`mt-2 p-2 text-sm rounded ${oracleTestResult.success ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}`}>
                      {oracleTestResult.message}
                    </div>
                  )}
                  
                  <p className="text-xs text-muted-foreground mt-1">
                    For testing, use: https://api.coingecko.com/api/v3/simple/price?ids={tokenSymbol}&vs_currencies=usd
                  </p>
                </div>
                  ...
                  <div className="flex justify-between">
                    <Label htmlFor="swapFee" className="flex items-center gap-2">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p className="max-w-xs">The percentage fee charged on each swap. Standard is 0.3%.</p>
                        </TooltipContent>
                      </Tooltip>
                    </Label>
                ...
                <div className="flex items-center justify-between space-x-2 rounded-md border p-4">
                  <div className="space-y-0.5">
                    <Label htmlFor="autoArbitrage" className="flex items-center gap-2">
                      Auto Arbitrage
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <HelpCircle className="h-4 w-4 text-muted-foreground" />
                        ...
                        <TooltipContent>
                          <p className="max-w-xs">Automatically arbitrage between oracle and pool prices to maintain price alignment.</p>
                        </TooltipContent>
                      </Tooltip>
                    </Label>
              ...
              <div className="rounded-md bg-muted p-4 flex items-start gap-3">
                ...
                <div className="text-sm">
                  <p className="font-medium mb-1">Oracle Maintenance Requirements</p>
                  <p className="text-muted-foreground">
                    This pool will require oracle price updates every 5 minutes. If the oracle fails to update, the pool will be automatically disabled until prices are refreshed.
                  </p>
                </div>
              </div>
            </CardContent>
